...
dt <- selectedLsoasDT[, .(LSOA11CD, WD18NM, IMD_Decile_label, All_Tco2e_per_dw = CREDStotal_kgco2e_pdw/1000)]
t <- summary(dt)
...
t <- selectedLsoasDT[, .(mean_gas_kgco2e = mean(CREDSgas_kgco2e2018_pdw),
...
t <- selectedLsoasDT[, .(mean_car_kgco2e = mean(CREDScar_kgco2e2018_pdw),
...
dt <- selectedLsoasDT[, .(LSOA11CD, LSOA01NM, WD18NM, T_CO2e_pdw = CREDStotal_kgco2e_pdw/1000, GBP_allEmissions_levy = beis_GBPtotal_c_perdw)]
...
dt <- selectedLsoasDT[, .(LSOA11CD, LSOA01NM,WD18NM, gas_T_CO2e_pdw = CREDSgas_kgco2e2018_pdw/1000, GBP_gas_levy_perdw = beis_GBPtotal_c_gas_perdw)]
...
dt <- selectedLsoasDT[, .(LSOA11CD, LSOA01NM, WD18NM, elec_T_CO2e_pdw = CREDSelec_kgco2e2018_pdw/1000, GBP_elec_levy_perdw = beis_GBPtotal_c_elec_perdw)]
...
```{r compareScenariosPlotAll, fig.cap="Comparing £ levy under each scenario"}
...
makeFlexTable(t, cap = "Scenario 2 totals (£ m)")

t <- selectedLsoasDT[, .(nLSOAs = .N, sumAllConsEmissions_GBP = sum(beis_GBPtotal_sc2)/m,
                sumGasEmissions_GBP = sum(beis_GBPgas_sc2)/m,
                sumElecEmissions_GBP = sum(beis_GBPelec_sc2)/m,
                sumPop = sum(pop_2018)), keyby = region]
makeFlexTable(t, cap = "Scenario 2 totals by region (£ m)")
