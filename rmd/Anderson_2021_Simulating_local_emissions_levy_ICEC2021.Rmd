...
# merge some of the LSOA labels to the boundaries so they can be in the map pop-ups
soton_sf <- merge(boundaries$lsoa_soton_leaflet, data$lsoa_lookup_solent[, .(LSOA11CD, WD20CD, WD20NM)],
                                       by = "LSOA11CD")
...
# merge to soton_sf to pick up ward names etc
map_sf <- merge(soton_sf, data$lsoa_imd2019_soton[, .(LSOA11CD= lsoa11cd, IMDScore, IMD_Decile,IMD_Decile_label)],
                                       by = "LSOA11CD")


colorPal <- colorNumeric("Reds", boundaries$lsoa_soton_leaflet$IMDScore, n = 9)
...
data$lsoa_beis_fp[, LSOA11CD := `LSOA Code`]

map_sf <- merge(soton_sf, data$lsoa_beis_fp[, .(LSOA11CD, 
...
```{r mapIMDallCO2eperdw, fig.cap="Annual mean T CO2e per dwelling for all Southampton LSOAs"}
sf <- merge(boundaries$lsoa_soton_leaflet, t, by = "LSOA11CD")

colorPal <- colorNumeric("Reds", t$All_Tco2e_per_dw, n = 10)

m <- leaflet(add_sf_WD20NM(sf)) %>%
  addTiles() %>%  # Add default OpenStreetMap map tiles
...
ggplot2::ggsave(filename = here::here("plots/sotonLSOAs_scatterCREDStotal_kgco2e_pdw_CREDSmeasuredHomeEnergy_kgco2e2018_pdw.png"))
```
...
m <- leaflet(add_sf_WD20NM(credsLsoaDT_sf)) %>%
  addTiles() %>%  # Add default OpenStreetMap map tiles
...
m %>%
    addLegend("bottomright", pal = colCats, 
              values = ~retrofitSum,
