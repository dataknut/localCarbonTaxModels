...
subtitle: "'Wider Solent' as a case study"
...
# Emissions Levy Case Study - Wider Solent
...
Load LSOA look-up table and list the local authroities we are covering.
...
credsLsoaDT[, .(meanRetrofitPerLSOA = mean(retrofitSum)/1000,
                totalRetrofit = sum(retrofitSum))]
```
...
summaryS1_T <- credsLsoaDT[, .(Total_Retrofit_Cost_k = sum(retrofitSum)/1000,
                        Total_All_emissions_Levy_k = sum(beis_GBPtotal_c)/1000,
                        Total_Gas_emissions_Levy_k = sum(beis_GBPtotal_c_gas)/1000,
                        Total_Elec_emissions_Levy_k = sum(beis_GBPtotal_c_elec)/1000,
                        Total_Metered_Energy_emissions_Levy_k = sum(beis_GBPtotal_c_meteredEnergy)/1000,
                        Total_Energy_emissions_Levy_k = sum(beis_GBPtotal_c_energy)/1000), keyby = .(LAD11NM)]

summaryS1_T[, Total_All_emissions_Levy_pc := 100*(Total_All_emissions_Levy_k/Total_Retrofit_Cost_k)]
summaryS1_T[, Total_Metered_emissions_Levy_pc := 100*(Total_Metered_Energy_emissions_Levy_k/Total_Retrofit_Cost_k)]
summaryS1_T[, Total_Energy_emissions_Levy_pc := 100*(Total_Energy_emissions_Levy_k/Total_Retrofit_Cost_k)]

makeFlexTable(summaryS1_T, cap = "Summary of Scenario 1 (totals, £ m GBP)")
...
summaryS2_T <- credsLsoaDT[, .(Total_Retrofit_Cost_k = sum(retrofitSum)/1000,
                        Total_All_emissions_Levy_k_sc2 = sum(beis_GBPtotal_sc2)/1000,
                        Total_Gas_emissions_Levy_k_sc2 = sum(beis_GBPgas_sc2)/1000,
                        Total_Elec_emissions_Levy_k_sc2 = sum(beis_GBPelec_sc2)/1000), keyby = .(LAD11NM)]

summaryS2_T[, Total_All_emissions_Levy_pc := 100*(Total_All_emissions_Levy_k_sc2/Total_Retrofit_Cost_k)]
...
Figure \@ref(fig:compareScenariosLevy) compares the total levy generated by district.

```{r compareScenariosLevy, fig.cap="Compare levy at district level"}
s1Totals_l <- melt(summaryS1_T)
s1Totals_l[, scenario := "Scenario 1"]
s2Totals_l <- melt(summaryS2_T)
s2Totals_l[, scenario := "Scenario 2 (rising block)"]
plotDT <- rbind(s1Totals_l, s2Totals_l)

ggplot2::ggplot(plotDT[variable %like% "Total_All_emissions_Levy_k"], aes(x = reorder(LAD11NM, -value), y = value/1000, fill = scenario)) + 
  geom_col(position = "dodge") +
  scale_fill_discrete(name = "Scenario") +
  labs(y = "Emissions levy £ m (Year 1)",
       x = "District",
       caption = "All emissions") +
  coord_flip()
```
Figure \@ref(fig:compareScenariosRetrofitCosts)

```{r compareScenariosRetrofitCosts, fig.cap="Compare districts"}

dt <- cbind(summaryS1_T, summaryS2_T)
mdt <- melt(dt[, .(LAD11NM,  
                   "Retrofit cost" = Total_Retrofit_Cost_k, 
                   "Year 1 levy (scenario 1)" = Total_All_emissions_Levy_k, 
                   "Year 1 levy (scenario 2)" =Total_All_emissions_Levy_k_sc2)])

ggplot2::ggplot(mdt, aes(x = reorder(LAD11NM, -value), 
                         y = value/1000, fill = variable)) + 
  geom_col(position = "dodge") +
