...
# run this code from the makefile
...
# A number of others will already be loaded by the makefile
rmdLibs <- c("flextable",
            "ggplot2",
            "leaflet",
            "lubridate",
...
This report is based on a model presented at the 2021 [International Conference on Evolving Cities](https://evolvingcities.org/), University of Southampton, 22 – 24 September 2021.
...
`r params$authors` (`r lubridate::year(today())`) [`r params$title`: `r rmdParams$filter` as a case study](`r reportLink`).  University of Southampton, United Kingdom
...
> Section \@ref(resultsSummary) gives the overall summary of the annual £ emissions levy value and estimated retrofit costs

 * Figure \@ref(fig:mapAllCO2eperdw) maps the LSOA level per dwelling 'all emissions' in Tonnes CO2e - which areas emit the most?
 ...
 * Figure \@ref(fig:compareScenariosBarAll) compares the scenarios in terms of % of levy generated by areas in each IMD decile while Figure \@ref(fig:compareScenariosPlotAll) compares the levy generated under each scenario at LSOA level. In both cases, Scenario 2 should be lower in more deprived areas and higher in less deprived areas. Figure \@ref(fig:compareScenariosBarAll) shows whether or not this is the case
 ...
 * Figure \@ref(fig:transferVisAllSc1) shows what would happen after year 1 if the levy were shared equally across LSOAs (all emissions, Scenario 1) and Figure \@ref(fig:transferVisAllSc2) shows the same for Scenario 2.
 * Figure \@ref(fig:mapYearsToPaySc1Allemissions) maps the years to pay under Scenario 1
...
This report estimates a model of an emissions levy for **`r rmdParams$filter`** using LSOA level data on emissions derived from the CREDS place-based emissions calculator. 
 ...
 * various LSOA/Ward boundary files and look-up tables from [geoportal](https://geoportal.statistics.gov.uk/search?collection=Dataset&sort=name&tags=all(LUP_CEN))
 
> All analysis is at LSOA level. Cautions on inference from area level data apply.

```{r filterData}
# filter data here based on param ----
  
#rmdParams$filter <- "East Suffolk"
...
message("Number of selected LSOAs - merged data:")
nrow(selectedLsoasDT)
table(selectedLsoasDT$case_study, selectedLsoasDT$`Local Authority Name`)
# selectedLsoasDT <- mergedLSOA_Data_Eng[LAD11NM %like% "East Suffolk"]

# from here onwards refer to the full LA name
# rmdParams$filter <- unique(selectedLsoasDT$`Local Authority Name`)

# test linkage of Census 2021 data - suspect some LSOAs have changed
message("Test linkage of Census 2021 data - suspect some LSOAs have changed")
selectedLsoasDT[, LSOA_2011CD := LSOA11CD]
setkey(selectedLsoasDT, LSOA11CD)
setkey(lsoa_2021_elec_ch, LSOA21CD)
test <- lsoa_2021_elec_ch[selectedLsoasDT[, .(LSOA11CD, WD18NM, CREDS_pc_Heating_Electric_2011, 
                                              IMDScore, pc_fuelPoor = as.numeric(`Proportion of households fuel poor (%)`))]]
summary(test[, .(c2021_nhhs, c2021_pc_elec_ch_2021, IMDScore, pc_fuelPoor)]) 
message("# NAs are LSOAs that didn't match - only matters to analysis using Census 2021 data on electricity central heating")
```

## Case study context

This report takes `r rmdParams$filter` as a case study. 

 * Number of households (Census 2021): `r dkUtils::tidyNum(sum(test$c2021_nhhs, na.rm = TRUE), round = 0)`
 * Number of electricity meters (2018): `r dkUtils::tidyNum(sum(selectedLsoasDT$nElecMeters), round = 0)`
  * Number of gas meters (2018): `r dkUtils::tidyNum(sum(selectedLsoasDT$nGasMeters), round = 0)`

Figure \@ref(fig:mapIMDLSOA) shows the distribution of deprivation across the case study area using the IMD 2019 data.
...
```{r mapIMDLSOA, fig.cap="IMD (LSOAs)"}

map_sf <- merge(boundaries$lsoa_2011_leaflet, selectedLsoasDT[, .(LSOA11CD,
                                                             IMDScore)], by = "LSOA11CD")

colorPal <- leaflet::colorNumeric("Reds", map_sf$IMDScore, n = 9)

m <- leaflet(map_sf) %>%
  addTiles() %>%  # Add default OpenStreetMap map tiles
  addPolygons(color = ~colorPal(IMDScore),
              smoothFactor = 0.2,
              fillOpacity = 0.6, 
              weight = 1.5, 
              popup = ~(paste0("LSOA: ",LSOA11NM,
                               "<br> Ward: ", WD20NM,
                               "<br> T CO2e/dwelling: ", dkUtils::tidyNum(IMDScore,0))
                      ), # popups clicked
...
Figure \@ref(fig:mapFuelPovertyLSOA) shows the distribution of fuel poverty across the case study area using the BEIS/DESNZ 2019 data.

```{r mapFuelPovertyLSOA, fig.cap="Fuel poverty (LSOAs)"}

#selected_lsoa_beis_fp <- lsoa_beis_fp[`LA Name` %like% "Suffolk"]

map_sf <- merge(boundaries$lsoa_2011_leaflet, selectedLsoasDT[, .(LSOA11CD, 
                                                fp_pc = as.numeric(`Proportion of households fuel poor (%)`))],
                                       by = "LSOA11CD")

colorPal <- colorNumeric("Reds", map_sf$fp_pc, n = 9)

m <- leaflet(map_sf) %>%
  addTiles() %>%  # Add default OpenStreetMap map tiles
  addPolygons(color = ~colorPal(fp_pc),
              fillOpacity = 0.6, 
              weight = 1.5, 
              popup = ~(paste0("LSOA: ",LSOA11NM,
                               "<br> Ward: ", WD20NM,
...
message("Number of EPC coded dwellings in case study area, (", rmdParams$filter, ") = ", sum(plotDT[case_study == 1]$value))
```

Imputed EPCs (see logic in Section \@ref(imputeEPCs)) - this corrects for numbers but not the bias in the EPC sample
...
message("Number of imputed EPC coded dwellings in case study area, (", rmdParams$filter, ") = ", round(sum(plotDT[case_study == 1]$value)))
...
We should not be surprised that emissions are negatively correlated with deprivation. If you are, you should try reading an [excellent paper by Milena Buchs and Sylke Schnepf](https://doi.org/10.1016/j.ecolecon.2013.03.007), both formerly of the University of Southampton:
...
```{r mapAllCO2eperdw, fig.cap="Annual mean T CO2e per dwelling (LSOAs)"}
map_sf <- merge(boundaries$lsoa_2011_leaflet, selectedLsoasDT[, .(LSOA11CD,
                                                             All_Tco2e_per_dw = CREDS_total_kgco2e_pdw/1000)], by = "LSOA11CD")

colorPal <- colorNumeric("Reds", map_sf$All_Tco2e_per_dw, n = 9)

m <- leaflet(map_sf) %>%
  addTiles() %>%  # Add default OpenStreetMap map tiles
  addPolygons(color = ~colorPal(All_Tco2e_per_dw),
              smoothFactor = 0.2,
              fillOpacity = 0.6, 
              weight = 1.5, 
              popup = ~(paste0("LSOA: ",LSOA11NM,
                               "<br> Ward: ", WD20NM,
                               "<br> T CO2e/dwelling: ", dkUtils::tidyNum(All_Tco2e_per_dw,0))
                      ), # popups clicked
              highlight = highlightOptions(
                weight = 5,
                color = "#666",
...
m %>%
    addLegend("bottomright", pal = colorPal, 
              values = ~All_Tco2e_per_dw,
              title = "Mean T Co2e",
              labFormat = labelFormat(suffix = "T CO2e"),
              opacity = 0.6
              )
...
```{r mapEnergyCO2eperdw, fig.cap="Annual mean T CO2e due to energy use per dwelling (LSOAs)"}
map_sf <- merge(boundaries$lsoa_2011_leaflet, selectedLsoasDT[, .(LSOA11CD, 
                                                             Tco2e_per_dw = CREDS_allHomeEnergy_kgco2e2018_pdw/1000)], by = "LSOA11CD")

colorPal <- colorNumeric("Reds", map_sf$Tco2e_per_dw, n = 9)

m <- leaflet(map_sf) %>%
  addTiles() %>%  # Add default OpenStreetMap map tiles
  addPolygons(color = ~colorPal(Tco2e_per_dw),
              smoothFactor = 0.2,
              fillOpacity = 0.6, 
              weight = 1.5, 
              popup = ~(paste0("LSOA: ",LSOA11NM,
                               "<br> Ward: ", WD20NM,
                               "<br> T CO2e/dwelling: ", dkUtils::tidyNum(Tco2e_per_dw,0))
                      ), # popups clicked
              highlight = highlightOptions(
                weight = 5,
                color = "#666",
...
ggplot2::ggplot(selectedLsoasDT, aes(x = IMDScore, y = CREDS_car_kgco2e2018_pdw/1000)) +
  ...
  scale_color_viridis_d(name = "IMD Decile") +
  labs(x = "IMD 2019 score",
       y = "Estimated Transport T CO2e/household (CREDS, 2021)",
       caption = capFilter)
...
```{r mapTransportCO2eperdw, fig.cap="Mean annual T CO2e per dwelling due to transport (LSOAs)"}
map_sf <- merge(boundaries$lsoa_2011_leaflet, selectedLsoasDT[, .(LSOA11CD,
                                                             Tco2e_per_dw = CREDS_personalTransport_kgco2e2018_pdw/1000)], by = "LSOA11CD")
...
Table \@ref(tab:scenario1Totals) below shows the overall £ GBP emissions levy total for the case study area in £M under Scenario 1.

```{r scenario1Totals, fig.cap="Emissions level due to total emissions, gas & electricity use by LSOA (Scenario 1)"}
m <- 1000000

levy_Sc1_t <- selectedLsoasDT[, .(nLSOAs = .N, "All emissions" = sum(beis_GBPtotal_c)/m,
                     "Gas" = sum(beis_GBPtotal_c_gas)/m,
                     "Electricity" = sum(beis_GBPtotal_c_elec)/m)]
makeFlexTable(levy_Sc1_t[order(-`All emissions`)], cap = "Total £m GBP levy by source (Scenario 1)")


t <- selectedLsoasDT[, .(nLSOAs = .N, "All emissions" = sum(beis_GBPtotal_c)/m,
                     "Gas" = sum(beis_GBPtotal_c_gas)/m,
                     "Electricity" = sum(beis_GBPtotal_c_elec)/m), keyby = .(WD20NM)]

makeFlexTable(t[order(-`All emissions`)], cap = "Total £m GBP levy by source, ordered by highest emitting ward (Scenario 1)")

makeFlexTable(t[order(-`All emissions`)], cap = "Total £m GBP levy by source, ordered by highest emitting ward (Scenario 1)")

mt <- melt(t)

ggplot2::ggplot(mt[variable != "nLSOAs"], aes(x = reorder(WD20NM, -value), y = value, fill = variable)) +
...
makeFlexTable(t, cap = "Mean £ GBP levy per dwelling by source (Scenario 1)")
...
#ggplot2::ggsave(filename = here::here("plots/allLSOAs_whoWins_emissions levy beis_GBPtotal_c.png"))
...
makeFlexTable(t, cap = "Highest emitting LSOAs by £ GBP gas levy")
t <- tail(dt[order(-GBP_gas_levy_perdw)])
makeFlexTable(t, cap = "Lowest emitting LSOAs by £ GBP gas levy")
```
...
makeFlexTable(t, cap = "Highest emitting LSOAs by £ GBP electricity levy")
t <- tail(dt[order(-GBP_elec_levy_perdw)])
makeFlexTable(t, cap = "Lowest emitting LSOAs by £ GBP electricity levy")
```
...
```{r scenario2Totals, fig.cap="Total levy due to all emissions, gas & electricity use by LSOA (Scenario 2)"}
m <- 1000000

levy_Sc2_t <- selectedLsoasDT[, .(nLSOAs = .N, "All emissions" = sum(beis_GBPtotal_sc2)/m,
                     "Gas" = sum(beis_GBPgas_sc2)/m,
                     "Electricity" = sum(beis_GBPelec_sc2)/m)]
makeFlexTable(levy_Sc2_t[order(-`All emissions`)], cap = "Total £m GBP levy by source (Scenario 2)")


t <- selectedLsoasDT[, .(nLSOAs = .N, "All emissions" = sum(beis_GBPtotal_sc2)/m,
                     "Gas" = sum(beis_GBPgas_sc2)/m,
                     "Electricity" = sum(beis_GBPelec_sc2)/m), keyby = .(WD20NM)]

makeFlexTable(t[order(-`All emissions`)], cap = "Total £m GBP levy by source, ordered by highest emitting ward (Scenario 2)")
...
t <- selectedLsoasDT[, .(All_emissions = mean(beis_GBPtotal_sc2_c_perdw),
                     Gas = mean(beis_GBPgas_sc2_perdw),
                     Electricity = mean(beis_GBPelec_sc2_perdw))]

makeFlexTable(t, cap = "Mean £ GBP levy per dwelling by source (Scenario 2)")
...
Source: estimates of the cost of retrofitting dwellings in different EPC bands ‘to an appropriate standard’ published in a recent English Housing Survey technical report ([MHCLG 2018](https://www.gov.uk/government/statistics/english-housing-survey-2018-energy-report)) (see https://journal-buildingscities.org/articles/10.5334/bc.279#T2)
...
retrofitCostsTotal <- selectedLsoasDT[, .(`Mean total £m per LSOA` = mean(retrofitSum)/mil,
                `Total £m` = sum(retrofitSum)/mil)]
makeFlexTable(retrofitCostsTotal, cap = paste0("Retrofit cost totals (£m GBP): ", rParams$filter))

retrofitCostsByWard <- selectedLsoasDT[, .(`Mean total £m per LSOA` = mean(retrofitSum)/mil,
                `Total £m` = sum(retrofitSum)/mil), keyby = .(Ward = WD18NM)]
makeFlexTable(retrofitCostsByWard[order(-`Total £m`)], 
              cap = paste0("Retrofit cost totals by ward (£m GBP): ", rParams$filter))
```
...
```{r mapRetrofitCosts, fig.cap="Total estimated retorfit costs per LSOA"}
map_sf <- merge(boundaries$lsoa_2011_leaflet, selectedLsoasDT[, .(LSOA11CD, 
                                                             cost = retrofitDE_cost + retrofitFG_cost)], by = "LSOA11CD")

colorPal <- colorNumeric("Reds", map_sf$cost, n = 9)

m <- leaflet(map_sf) %>%
  addTiles() %>%  # Add default OpenStreetMap map tiles
  addPolygons(color = ~colorPal(cost),
              smoothFactor = 0.2,
              fillOpacity = 0.6, 
              weight = 1.5, 
              popup = ~(paste0("LSOA: ",LSOA11NM,
                               "<br> Ward: ", WD20NM,
                               "<br> T CO2e/dwelling: ", dkUtils::tidyNum(cost,0))
                      ), # popups clicked
              highlight = highlightOptions(
                weight = 5,
                color = "#666",
...
Figure \@ref(fig:mapYearsToPaySc1Allemissions) maps the distribution of years to pay for all LSOAs in the case study.

```{r mapYearsToPaySc1Allemissions, fig.cap="Years to pay per LSOA (Scenbario 1, all emissions levy, no redistribution)"}
map_sf <- merge(boundaries$lsoa_2011_leaflet, selectedLsoasDT[, .(LSOA11CD, 
                                                             y2p = yearsRequiredToPayRetrofit_beis_GBPtotal_c_perdw)], by = "LSOA11CD")

colorPal <- colorNumeric("Reds", map_sf$y2p, n = 9)

m <- leaflet(map_sf) %>%
  addTiles() %>%  # Add default OpenStreetMap map tiles
  addPolygons(color = ~colorPal(y2p),
              smoothFactor = 0.2,
              fillOpacity = 0.6, 
              weight = 1.5, 
              popup = ~(paste0("LSOA: ",LSOA11NM,
                               "<br> Ward: ", WD20NM,
                               "<br> Years to pay: ", dkUtils::tidyNum(y2p,0))
                      ), # popups clicked
              highlight = highlightOptions(
                weight = 5,
                color = "#666",
...
Figure \@ref(fig:sc1Y1surplusByLsoaAllEmissions) shows those LSOAs which would be in surplus (above the y = 0 line) after Year 1.

```{r sc1Y1surplusByLsoaAllEmissions, fig.cap="Surplus £ after year 1 for each LSOA (all emissions levy)"}
...
Figure \@ref(fig:sc1Y1surplusByLsoaEnergyEmissions) shows the same distribution but for the energy emissions levy. Clearly there is a lower level of surplus due to the reduced levy value at LSOA level.

```{r sc1Y1surplusByLsoaEnergyEmissions, fig.cap="Surplus £ after year 1 for each LSOA (energy levy)"}
...
Figure \@ref(fig:redistVisAllEmissionsSc1) shows the year 1 outcome at ward level if levy is shared equally across all LSOAs (all emissions levy - orange dots).

```{r redistVisAllEmissionsSc1, fig.cap="Year 1 outcome if levy is shared equally (all emissions levy)"}
...
ggplot2::ggplot(plotDT, aes(x = reorder(LSOA11CD, -retrofitSum), fill = retrofitSum/1000000)) +
  ...
  scale_color_discrete(name = "All emissions levy Year 1 (£M) \nif equally re-distributed") +
  facet_wrap(. ~ WD18NM, scales="free_x") +
  labs(x = "LSOAs (ordered by retrofit cost)",
       y = "£M",
       caption = "BEIS central carbon cost, all emissions levy (Scenario 1)")
# 
ggplot2::ggsave(paste0(rmdParams$plotPath, "BC_r1_Figure_9_sc1.jpg"))
...
```{r highestRetrofitCostSc1}
t <- head(selectedLsoasDT[, .(LSOA = LSOA11CD, "LSOA name" = LSOA11NM, Ward = WD18NM, 
                              `Retrofit cost £m` = retrofitSum/1000000,
                              "Years to pay (all emissions levy)" = yearsRequiredToPayRetrofit_beis_GBPtotal_c_perdw, 
                              "Years to pay (energy emissions levy)" = yearsRequiredToPayRetrofit_beis_GBPtotal_c_energy_perdw, 
                              "% EPC D" = epc_D_pc*100, "% EPC E" =  epc_E_pc*100, 
                              "% EPC F" = epc_F_pc*100, "% EPC G" = epc_G_pc*100)][order(-`Retrofit cost £m`)],10)
makeFlexTable(t, cap = "Longest years to pay under Scenario 1 (sorted by total retrofit cost)")
```
...
```{r allLevyPayOffSc2, fig.height=5, fig.cap="Years to pay under Scenario 2 (all emissions)"}
...
ggplot2::ggplot(selectedLsoasDT, aes(x = IMDScore, 
  ...
  scale_color_viridis_d(name = "IMD Decile") +
  labs(y = "Years to pay off",
       caption = paste0("Rising block, all emissions levy (Scenario 10",
                        "\n", capFilter))
...
summary(selectedLsoasDT$yearsRequiredToPayRetrofit_beis_GBPtotal_sc2_energy_perdw)
...
Figure \@ref(fig:sc2Y1surplusByLsoaAllEmissions) shows those LSOAs which would be in surplus (above the y = 0 line) after Year 1.

```{r sc2Y1surplusByLsoaAllEmissions, fig.cap="Surplus £ after year 1 for each LSOA (Scenario 2, all emissions levy)"}
...
Figure \@ref(fig:sc2Y1surplusByLsoaEnergyEmissions) shows the same distribution for the Scenario 2 energy levy.

```{r sc2Y1surplusByLsoaEnergyEmissions, fig.cap="Surplus £ after year 1 for each LSOA (Scenario 2, all emissions levy)"}
ggplot2::ggplot(plotDT, aes(x = reorder(LSOA11CD, -retrofitSum), colour = IMD_Decile_label,
  ...
  labs(x = "LSOAs (ordered by retrofit cost)",
       y = "Year 1 surplus (£M)",
       caption = paste0("Scenario 2, energy emissions levy",
                        "\n", capFilter))
...
Figure \@ref(fig:redistVisAllEmissionsSc2) shows the year 1 outcome at ward level if levy is shared equally across all LSOAs (all emissions levy - orange dots).

```{r redistVisAllEmissionsSc2, fig.cap="Year 1 outcome if levy is shared equally (Scenario 2, all emissions levy)"}
...
ggplot2::ggplot(plotDT, aes(x = reorder(LSOA11CD, -retrofitSum), fill = retrofitSum/1000000)) +
  ...
  labs(x = "LSOAs (ordered by retrofit cost)",
       y = "£M",
       caption = "BEIS central carbon cost, all emissions levy (Scenario 2)")
# 
ggplot2::ggsave(paste0(rmdParams$plotPath, "BC_r1_Figure_9_sc2.jpg"))
...
```{r highestRetrofitCostSc2}
t <- head(selectedLsoasDT[, .(LSOA = LSOA11CD, "LSOA name" = LSOA11NM, Ward = WD18NM, 
                              `Retrofit cost £m` = retrofitSum/1000000,
                              "Years to pay (all emissions levy)" = yearsRequiredToPayRetrofit_beis_GBPtotal_sc2_perdw, 
                              "Years to pay (energy emissions levy)" = yearsRequiredToPayRetrofit_beis_GBPtotal_sc2_energy_perdw, 
                              "% EPC D" = epc_D_pc*100, "% EPC E" =  epc_E_pc*100, 
                              "% EPC F" = epc_F_pc*100, "% EPC G" = epc_G_pc*100)][order(-`Retrofit cost £m`)],10)
makeFlexTable(t, cap = "Longest years to pay under Scenario 2 (sorted by total retrofit cost)")
```
...
# Summary: `r rmdParams$filter` {#resultsSummary}

Table \@ref(tab:summaryLevyResults) shows the summary overall levy value results for `r rmdParams$filter` by Scenario.

```{r summaryLevyResults}

mil <- 1000000

levy_Sc1_t[, Scenario := "Scenario 1"]
levy_Sc2_t[, Scenario := "Scenario 2"]

t <- rbind(levy_Sc1_t,levy_Sc2_t)

makeFlexTable(t[, .(`Scenario`, `nLSOAs`, 
                    "All emissions levy" = `All emissions`, 
                    "Gas emissions levy" = `Gas`, 
                    "Electricity emissions levy" = `Electricity`)], cap = paste0("Levy total (Year 1 £m GBP): ", rmdParams$filter))

levy_t1 <- selectedLsoasDT[, .(nLSOAs = .N, "All emissions levy (£m, Scenario 1)" = sum(beis_GBPtotal_c)/mil,
                     "Gas emissions levy (£m, Scenario 1)" = sum(beis_GBPtotal_c_gas)/mil,
                     "Electricity emissions levy (£m, Scenario 1)" = sum(beis_GBPtotal_c_elec)/mil), keyby = .(WD20NM)]

levy_t2 <- selectedLsoasDT[, .("All emissions levy (£m, Scenario 2)" = sum(beis_GBPtotal_sc2)/mil,
                     "Gas emissions levy (£m, (Scenario 2)" = sum(beis_GBPgas_sc2)/mil,
                     "Electricity emissions levy (£m, Scenario 2)" = sum(beis_GBPelec_sc2)/mil), keyby = .(WD20NM)]
levyResByWard <- levy_t1[levy_t2]

```

Table \@ref(tab:summaryRetrofitCostResults) shows the summary overall retrofit cost results for `r rmdParams$filter`.

```{r summaryRetrofitCostResults}
retrofitCostsTotal <- selectedLsoasDT[, .(`Total retrofit cost £m` = sum(retrofitSum)/mil,
                                          `Mean total retrofit cost £m per LSOA` = mean(retrofitSum)/mil
                                          )
                                      ]
makeFlexTable(retrofitCostsTotal, cap = paste0("Retrofit cost totals (£m GBP): ", rmdParams$filter))

retrofitCostsByWard <- selectedLsoasDT[, .(`Total retrofit cost (£m)` = sum(retrofitSum)/mil, 
                                           `Mean total retrofit cost per LSOA (£m)` = mean(retrofitSum)/mil), 
                                       keyby = .(Ward = WD18NM)]

modRes <- dkUtils::tidyNum(retrofitCostsTotal$`Total retrofit cost £m`, round = 2)

if(rmdParams$filter %like% "Islington"){
  t <- paste0("The model is similar in some respects to Evans et al (2023) *Getting to net zero: Islington’s social housing stock* (https://journal-buildingscities.org/articles/10.5334/bc.349) which estimated an overall cost of upgrading Islington's *social housing stock* (only) of 4500 buildings containing some 33,300 dwellings to be £1,600m, compared to this model's estimate of £", modRes, "m for *all* required dwellings.")
} else {
  t <- ""
}
```
 ...
 * Number of households (Census 2021): `r dkUtils::tidyNum(sum(test$c2021_nhhs, na.rm = TRUE), round = 0)`
 * Estimated number of domestic electricity meters (2018): `r dkUtils::tidyNum(sum(selectedLsoasDT$nElecMeters), round = 0)`
 * Estimated number of domestic gas meters (2018): `r dkUtils::tidyNum(sum(selectedLsoasDT$nGasMeters), round = 0)`
  
`r t`

The following table (Table \@ref(tab:megaTable)) summarises all results by ward.

```{r megaTable}

mt <- levyResByWard[retrofitCostsByWard]

makeFlexTable(mt[order(-`Total retrofit cost (£m)`)], 
              cap = paste0("All results by ward (£m GBP, sorted by total retrofit cost): ", rParams$filter))
