 ...
 * Figure \@ref(fig:mapYearsToPaySc1_allemissions) maps the years to pay under Scenario 1
...
makeFlexTable(levy_Sc1_t[order(-`All emissions`)], cap = "Total £m GBP levy by source (Scenario 2)")


t <- selectedLsoasDT[, .(nLSOAs = .N, beis_total_c = sum(beis_GBPtotal_c)/m,
                     beis_total_c_gas = sum(beis_GBPtotal_c_gas)/m,
                     beis_total_c_elec = sum(beis_GBPtotal_c_elec)/m), keyby = .(WD20NM)]

makeFlexTable(t[order(-beis_total_c)], cap = "Total £m GBP levy by source, ordered by highest emitting ward (Scenario 2)")

mt <- melt(t)
mt[, source := ifelse(variable == "beis_total_c", "All emissions", NA)]
mt[, source := ifelse(variable == "beis_total_c_gas", "Gas emissions", source)]
mt[, source := ifelse(variable == "beis_total_c_elec", "Electricity emissions", source)]
ggplot2::ggplot(mt[variable != "nLSOAs"], aes(x = reorder(WD20NM, -value), y = value, fill = source)) +
...
t <- selectedLsoasDT[, .(nLSOAs = .N, beis_total_c = sum(beis_GBPtotal_sc2)/m,
                     beis_total_c_gas = sum(beis_GBPgas_sc2)/m,
                     beis_total_c_elec = sum(beis_GBPelec_sc2)/m), keyby = .(WD20NM)]
...
```{r mapYearsToPaySc1_allemissions, fig.cap="Years to pay per LSOA (Scenbario 1, all emissions levy, no redistribution)"}
map_sf <- merge(boundaries$lsoa_2011_leaflet, selectedLsoasDT[, .(LSOA11CD, 
