...
summary(map_sf$CREDStotal_kgco2e_percap)
```
...
# sum of LSOA level CO2e for comparison with own estimates
data$lsoa_creds_pbcc[, total_kgco2e_gas := gas_percap_2018 * pop_2018]
# filter
d <- getSoton(data$lsoa_creds_pbcc)
message("Total CO2e per LSOA from residential gas 2018 (kg):")
summary(d$total_kgco2e_gas)
message("Total CO2e for Southampton from residential gas 2018 (T):")
s <- sum(d$total_kgco2e_gas/1000)
tidyNum(s)

sotonTotalsTCO2e <- list()
sotonTotalsTCO2e$CREDS_gas <- s
```
...
# sum of LSOA level CO2e for comparison with own estimates
data$lsoa_creds_pbcc[, total_kgco2e_elec := elec_percap_2018 * pop_2018]
d <- getSoton(data$lsoa_creds_pbcc)
message("Total CO2e per LSOA from residential electricity 2018 (kg):")
summary(d$total_kgco2e_elec)
message("Total CO2e for Southampton from residential electricity 2018 (T):")
s <- sum(d$total_kgco2e_elec/1000)
tidyNum(s)
```

```{r credsLSOAMapEnergyPC, fig.cap = "LSOA CREDS gas, electricity and other heat emissions as % of total per capita (shows LSOA, MSOA, ward names and % when clicked"}
...
ggplot2::ggsave(filename = here::here("plots/scatterIMD_CREDStotal_kgco2e_percap.png"))
```
...
ggplot2::ggsave(filename = here::here("plots/scatterIMD_CREDSgas_percap_2018.png"))
```
...
library(viridisLite)
ggplot2::ggplot(map_sf, aes(x = IMDScore, y = CREDSelec_percap_2018/1000)) +
  geom_point(aes(size = pc_Heating_Electric, colour = IMD_Decile_label)) +
  geom_smooth() +
  scale_size_continuous(name = "% Electric heating") +
  scale_color_viridis_d(name = "IMD Decile") +
...
ggplot2::ggsave(filename = here::here("plots/scatterIMD_CREDSelec_percap_2018.png"))
```

Now do the same for the % of emissions from gas & electricity and other heat - this shows the % emissions left to deal with after we (may) have fixed those.
...
ggplot2::ggplot(map_sf, aes(x = IMDScore, y = energy_pc)) +
  ...
  scale_size_continuous(name = "% Electric heating") +
  labs(x = "IMD 2019 score",
       y = "% total emissions which are gas, electricity or other heat (CREDS, 2021)")
ggplot2::ggsave(filename = here::here("plots/scatterIMD_energy_pc.png"))
```
## Fuel poverty
2019 estimates
