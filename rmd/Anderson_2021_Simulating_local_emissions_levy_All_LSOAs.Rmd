...
data$lsoa_lookup <- data.table::fread(paste0("~/Dropbox/data/UK_census/lookups/", "lsoa_lookup.csv"))

nrow(data$lsoa_lookup)

skimr::skim(data$lsoa_lookup)

table(data$lsoa_lookup$RUC11, data$lsoa_lookup$`Region/Country`, useNA = "always")
...
# reduce to LA sub-sample here if needed using LAD11NM %like% "XYZ"

# start from the lookup as it has all LSOAs/SOAs

lsoa_lookup <- data$lsoa_lookup
...
head(credsLsoaDT[, .(LSOA11NM, WD18NM, nGasMeters, nElecMeters, epc_total)][order(-nGasMeters)])

head(credsLsoaDT[, .(LSOA11NM, WD18NM, nGasMeters, nElecMeters, epc_total)][order(-nElecMeters)])

# keep only clean, allocated data
credsLsoaDT <- credsLsoaDT[!is.na(LSOA01NM)]
head(credsLsoaDT[, .(LSOA11NM, WD18NM, nGasMeters, nElecMeters, epc_total)][order(-nGasMeters)])
```
...
m <- 1000000
message("£m")
credsLsoaDT[, .(nLSOAs = .N, beis_GBPtotal_c = sum(beis_GBPtotal_c)/m,
                     beis_total_c_gas = sum(beis_GBPtotal_c_gas)/m,
                     beis_GBPtotal_c_elec = sum(beis_GBPtotal_c_elec)/m)]
message("£m")
t <- credsLsoaDT[, .(nLSOAs = .N, beis_GBPtotal_c = sum(beis_GBPtotal_c)/m,
                     beis_total_c_gas = sum(beis_GBPtotal_c_gas)/m,
...
t <- credsLsoaDT[, .(LSOA11CD, LSOA01NM, WD18NM, IMD_Decile_label, CREDStotal_kgco2e_pdw, beis_GBPtotal_c_perdw)]

flextable(head(t[order(-beis_GBPtotal_c_perdw)]))

flextable(tail(t[order(-beis_GBPtotal_c_perdw)]))
```
...
t <- credsLsoaDT[, .(LSOA11CD, LSOA01NM,WD18NM, IMD_Decile_label, gasTCO2e_pdw = CREDSgas_kgco2e2018_pdw/1000, beis_GBPtotal_c_gas_perdw)]


flextable(head(t[order(-beis_GBPtotal_c_gas_perdw)]))

flextable(tail(t[order(-beis_GBPtotal_c_gas_perdw)]))
```
...
t <- credsLsoaDT[, .(LSOA11CD, LSOA01NM, WD18NM, IMD_Decile_label, elecTCO2e_pdw = CREDSelec_kgco2e2018_pdw/1000, beis_GBPtotal_c_elec_perdw)]


flextable(head(t[order(-beis_GBPtotal_c_elec_perdw)]))

flextable(tail(t[order(-beis_GBPtotal_c_elec_perdw)]))
```
...
message("£m")
m <- 1000000
credsLsoaDT[, .(nLSOAs = .N, sumAllConsEmissions_GBP = sum(beis_GBPtotal_sc2)/m,
                sumGasEmissions_GBP = sum(beis_GBPgas_sc2)/m,
                sumElecEmissions_GBP = sum(beis_GBPelec_sc2)/m,
                sumPop = sum(pop_2018))]
message("£m")

t <- credsLsoaDT[, .(nLSOAs = .N, sumAllConsEmissions_GBP = sum(beis_GBPtotal_sc2)/m,
                sumGasEmissions_GBP = sum(beis_GBPgas_sc2)/m,
                sumElecEmissions_GBP = sum(beis_GBPelec_sc2)/m,
...
credsLsoaDT[, retrofitMeanPerDw := retrofitSum/(epc_D_imp + epc_E_imp + epc_F_imp + epc_G_imp)] # might create an NA if 
```

```{r reportRetrofitCosts}
mil <- 1000000
message("To retrofit D-E (£m)")
sum(credsLsoaDT$retrofitDE)/mil
message("Number of dwellings: ", round(sum(credsLsoaDT$sumDE)))

message("To retrofit F-G (£m)")
sum(credsLsoaDT$retrofitFG)/mil
message("Number of dwellings: ", round(sum(credsLsoaDT$sumFG)))

message("To retrofit D-G (£m)")
sum(credsLsoaDT$retrofitSum)/mil

message("To retrofit D-G (mean per dwelling)")
mean(credsLsoaDT$retrofitMeanPerDw, na.rm = TRUE)

credsLsoaDT[, .(meanPerLSOA_GBPm = mean(retrofitSum)/mil,
...
```{r transferVisAllSc1}
plotDT <- credsLsoaDT[, .(LSOA11CD, beis_GBPtotal_c, beis_GBPtotal_c_elec, beis_GBPtotal_c_gas, retrofitSum, WD18NM, IMD_Decile_label, IMDScore)]
...
head(credsLsoaDT[, .(LSOA11CD,LSOA11NM, WD18NM, retrofitSum,yearsToPay = yearsRequiredToPayRetrofit_beis_GBPtotal_c_energy_perdw, epc_D_pc, epc_E_pc, epc_F_pc, epc_G_pc)][order(-retrofitSum)],10)
```
...
```{r transferVisAllSc2}
plotDT <- credsLsoaDT[, .(LSOA11CD, beis_GBPtotal_sc2, beis_GBPelec_sc2, beis_GBPgas_sc2, retrofitSum, WD18NM, IMDScore, IMD_Decile_label)]
